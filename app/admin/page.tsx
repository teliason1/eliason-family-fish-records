...
export default async function AdminPage() {
  if (!isSupabaseConfigured()) return <main className="admin-page"><span className="kicker">Admin</span><h1>Moderation queue</h1><div className="setup-notice">Connect Supabase and apply <code>supabase/schema.sql</code> to activate moderation.</div><Link href="/concepts/atlas">← Back to the archive</Link></main>;
  const supabase = await createClient(); const { data: { user } } = await supabase!.auth.getUser(); if (!user) redirect("/login");
  const { data: profile } = await supabase!.from("profiles").select("role").eq("id", user.id).single(); if (profile?.role !== "admin") redirect("/concepts/atlas");
